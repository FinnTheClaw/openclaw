...
- Before upstream, cherry-pick, or semantic-port work, read `governance/HARDENING_ARCHITECTURE_POLICY.md` and `governance/hardening-dni.json`. DNI means no source, test, prompt, or schema code copy; only explicitly allowed independently implemented semantic assertions.
