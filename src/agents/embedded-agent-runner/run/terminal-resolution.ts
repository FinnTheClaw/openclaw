...
export function resolveSettledTurnFinalizationRequest(input: {
  ...
  const rejectedTerminal =
    hasRejectedPostToolTerminalText(input.attempt) &&
    !hasAttemptTerminalState(input.attempt) &&
    resolveSettledToolBatchEvidence(input.attempt).failedToolNames.size === 0;
  const hasOnlySyntheticErrorPayload = Boolean(
    (rejectedTerminal || input.attempt.assistantTexts.every((text) => text.trim().length === 0)) &&
    ...
    input.payloadsWithToolMedia?.every((payload) => {
      ...
      return (
        ...
        ((input.attempt.assistantTexts.every((text) => !text.trim()) &&
          metadata?.toolErrorWarning) ||
          ((input.attempt.terminal.kind === "failed" || rejectedTerminal) &&
