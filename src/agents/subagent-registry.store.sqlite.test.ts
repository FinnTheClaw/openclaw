...
function createRun(overrides: Partial<SubagentRunRecord> = {}): SubagentRunRecord {
  return {
    ...
    originalTask: "complete the root assignment",
    steeringMessages: ["use the bounded writer", "compile before completion"],
    steeringHistoryOmittedCount: 1,
    cleanup: "keep",
    createdAt: 100,
    startedAt: 110,
    endedAt: 250,
  ...
  it("persists subagent runs in the shared sqlite state database", async () => {
    await withTempStateEnv(async () => {
      ...
      expect(restored.get(run.runId)).toMatchObject({
        ...
        steeringMessages: run.steeringMessages,
        steeringHistoryOmittedCount: run.steeringHistoryOmittedCount,
        endedAt: run.endedAt,
        outcome: run.outcome,
