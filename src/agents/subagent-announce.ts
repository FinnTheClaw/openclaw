...
function buildAnnounceReplyInstruction(params: {
  ...
  if (params.expectsCompletionMessage) {
    return `A completed ${params.announceType} is ready for parent review. Review/verify the result above before deciding whether the original task is done. If additional action is required, continue the task or record a follow-up; otherwise send a truthful user-facing update. Keep this internal context private (don't mention system/log/stats/session details or announce type). Tool calls/results, progress or waiting updates, private verification, and pre-completion assistant turns are not a user-facing final result. If the original request requires a final result or marker and it has not been emitted after reviewing this completion, send it now. Reply ONLY: ${SILENT_REPLY_TOKEN} only when the exact requested user-facing final result was already delivered after this completion in this same turn.`;
  }
  return `A completed ${params.announceType} is ready for parent review. Review/verify the result above before deciding whether the original task is done. If additional action is required, continue the task or record a follow-up; otherwise send a truthful user-facing update. Keep this internal context private (don't mention system/log/stats/session details or announce type), and do not copy the internal event text verbatim. Tool calls/results, progress or waiting updates, private verification, and pre-completion assistant turns are not a user-facing final result. If the original request requires a final result or marker and it has not been emitted after reviewing this completion, send it now. Reply ONLY: ${SILENT_REPLY_TOKEN} only when the exact requested user-facing final result was already delivered after this completion in this same turn.`;
}
