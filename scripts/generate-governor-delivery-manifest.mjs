...
const artifactSource = artifacts
  .map((artifact) => {
    const property = `  ${JSON.stringify(artifact.path)}: ${JSON.stringify(artifact.sha256)},`;
    return property.length <= 100
      ? property
...
const generated =
  ...
  `const GOVERNOR_DELIVERY_BUILD_ARTIFACT_DIGESTS = {\n${artifactSource}\n} as const;\n` +
  `const GOVERNOR_DELIVERY_BUILD_ARTIFACTS = Object.entries(\n` +
  `  GOVERNOR_DELIVERY_BUILD_ARTIFACT_DIGESTS,\n` +
