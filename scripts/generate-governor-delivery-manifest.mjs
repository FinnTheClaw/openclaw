...
const outputJsonPath = path.join(
  root,
  "src/security/governor-host-delivery-build-manifest.generated.json",
);
const entrypoints = [
...
const generatedJson = `${JSON.stringify(normalizeJson({ digest: manifestDigest, manifest }))}\n`;
const generatedTs =
...
if (process.argv.includes("--verify")) {
  const currentTs = fs.existsSync(outputTsPath) ? fs.readFileSync(outputTsPath, "utf8") : "";
  const currentJson = fs.existsSync(outputJsonPath) ? fs.readFileSync(outputJsonPath, "utf8") : "";
  if (currentTs !== generatedTs || currentJson !== generatedJson) {
    console.error("Governor delivery build manifest is stale. Run pnpm governor:manifest:gen.");
    process.exit(1);
  }
} else {
  fs.writeFileSync(outputJsonPath, generatedJson);
