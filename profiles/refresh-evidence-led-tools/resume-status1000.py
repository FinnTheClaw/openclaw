"""Resume only missing status campaign submissions; preserve existing records."""
import argparse
...
HELPER = Path("/workspace/reports/reliability-resume-20260918/native-status1000.py")
...
CANDIDATE = Path("/workspace/malformed-finalization-runtime-c74d3d54")
CANDIDATE_REVISION = "c74d3d54ff3934096b910c07270d1e07b3cddc5e"
...
def main():
    ...
    os.umask(0o077)
    assert args.output.is_dir(), "Original campaign directory is required"
    ...
    assert json.loads((args.output / "source-config.json").read_text()) == base, "Config changed"
    ...
    manifest = {
        ...
        "arms": ["baseline", "candidate"], "cases": keys, "repetitionsPerCase": 500,
        "submissionsPerArm": 500, "totalSubmissions": 1000, "concurrency": 3,
        "sharedCompute": True, "latencyIsAcceptanceMetric": False, "cliTimeoutSeconds": 86400,
        "callerRetries": 0, "treatment": "Continuation treatment c74d3d54 versus pre-continuation c5ddb679; no rejected guidance profile; unchanged native retry counts",
        ...
        "scoring": "status.txt shipped; no other workspace changes; truthful visible confirmation; settled completion; no duplicate successful mutation",
        "timeoutPolicy": "Queue/transport limits are infrastructure outcomes, not model correctness failures; retain every outcome",
        "promotion": "Candidate perfect on all five criteria and observed paired improvement; ties do not prove benefit; no universal reliability claim",
    }
    manifest["resumes"] = json.loads((args.output / "manifest.json").read_text())["startedAt"]
    native.publish(args.output, "resume-01-manifest.json", manifest)
    for arm in manifest["arms"]:
        (args.output / arm).mkdir(exist_ok=True)
        native.publish(args.output / arm, "resume-01-manifest.json", {**manifest, "arm": arm})
    ...
    def run_pair(rep, index, case):
        ...
        for arm in arms:
            ...
            existing = folder / "turn-01" / "record.json"
            if existing.exists():
                prior = json.loads(existing.read_text())
                assert prior["identity"] == identity and prior["arm"] == arm
                rows.append(prior)
                continue
            assert not folder.exists(), "Partial turn needs explicit inspection, not replay"
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=3) as pool:
        futures = [pool.submit(run_pair, rep, index, case)
                   for rep in range(1, 501) for index, case in enumerate(selected)]
        for future in concurrent.futures.as_completed(futures):
            rows.extend(future.result())
    for arm in manifest["arms"]:
        native.publish(args.output / arm, "resume-01-completed.json",
                       {"at": native.now(), "records": [r for r in rows if r["arm"] == arm]})
    native.publish(args.output, "resume-01-completed.json", {"at": native.now(), "submissions": len(rows)})
