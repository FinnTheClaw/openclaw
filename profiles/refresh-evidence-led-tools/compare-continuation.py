"""Paired source-only continuation replay; real Qwen with a1024-token output budget."""
import argparse
...
CANDIDATE = Path("/workspace/settled-continuation-runtime-3630720b")
CANDIDATE_REVISION = "3630720bfa99ad47527f6633bde5fe3656a7a104"
...
def main():
    ...
    keys = ["R06-append-once", "R13-missing-source",
            "R20-two-turn-change", "D14-multiple-matches"]
    selected = []
    for key in keys:
        case = by_id[key]
        ...
        if key == "R20-two-turn-change":
            ...
            prompt = ("First use exec to locate status.txt within this workspace, then read it and "
                      "change ready to shipped. Confirm the changed contents. Do not change other files.")
    ...
    base["agents"]["defaults"]["models"]["finn-coordinator/moira/brain"]["params"]["maxTokens"] = 1024
    ...
    manifest = {
        ...
        "candidatePackage": str(CANDIDATE), "maxTokens": 1024,
        "helperSha256": sha(HELPER), "runnerSha256": sha(Path(__file__)),
        "configSha256": sha(native.SOURCE_CONFIG), "pluginsSha256": sha(PLUGINS),
        "arms": ["baseline", "candidate"], "cases": keys, "repetitionsPerCase": 3,
        "submissionsPerArm": 12, "totalSubmissions": 24, "concurrency": 1,
        "callerRetries": 0, "treatment": "Source-only3630720b; no guidance profile; unchanged native retry budgets",
        "caseInputsSha256": hashlib.sha256(json.dumps(selected, sort_keys=True).encode()).hexdigest(),
        "ordering": "Case pairs serial A/B or B/A, alternated by case+repetition; one source-only pair at a time alongside the winding-down guidance pilot",
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
        futures = [pool.submit(run_pair, rep, index, case)
                   for rep in range(1, 4) for index, case in enumerate(selected)]
